...
const normalizeJoinedGroups = (groups: JoinedGroupOption[]): JoinedGroupOption[] => {
...
const createComposeDraftId = (fromName: string, toName: string, updatedAt: number): string => {
...
export const NewMessage = ({
  ...
  const [threadPublishResult, setThreadPublishResult] = useState<ThreadPublishResult | null>(
    null
  )
  ...
  const [replyPreviewMode, setReplyPreviewMode] = useState<'preview' | 'full' | 'hidden'>(
    'preview'
  )
  ...
  const resolvedTarget = useMemo(() => resolveComposeTarget(), [resolveComposeTarget])
  const isGroupTarget = resolvedTarget?.type === 'group'
  const allowAliasAndBcc = !isGroupTarget
  const activeDraftKey = useMemo(() => {
    const senderName = fromName.trim()
    if (!user?.address || !senderName || resolvedTarget?.type !== 'name') return null
  ...
  const clearStoredDraft = useCallback(
    (draftKey?: string | null) => {
      ...
      const normalizedDraftKey = typeof draftKey === 'string' ? draftKey.trim() : ''
  ...
  const closeModal = useCallback(() => {
    ...
    onRequestClose?.()
  }, [inlineMode, onRequestClose, resetComposerDraft, setForwardInfo, setReplyTo])
  ...
  useEffect(() => {
    ...
    if (composePrefill.fromName) {
      const matchingFrom = fromOptions.find(option => {
        return normalizeValue(option) === normalizeValue(composePrefill.fromName || '')
      })
  ...
  useEffect(() => {
    ...
    const storedDraft = readComposeDraftsFromStorage(user.address)[activeDraftKey]
    ...
    setBccNames(Array.isArray(storedDraft.bccNames) ? storedDraft.bccNames : [])
    window.setTimeout(() => {
  ...
  useEffect(() => {
    if (!activeDraftKey || !user?.address || isHydratingDraftRef.current) return

    const timeout = window.setTimeout(() => {
      const fromNameValue = fromName.trim()
      const toNameValue = resolvedTarget?.type === 'name' ? resolvedTarget.label.trim() : ''
  ...
  const activeReplyPreviewText = useMemo(() => {
    ...
    return replyPreviewMode === 'full' ? replyBodyText || '- no message body -' : replyPreviewText
  ...
  const buildAttachmentPayloads = useCallback(
    async (publisherName: string): Promise<{
  ...
  const fetchGroupPublicKeys = useCallback(async (groupId: string): Promise<string[]> => {
    const normalizedGroupId = groupId.trim()
    if (!normalizedGroupId) return []

    const response = await fetch(`/groups/members/${encodeURIComponent(normalizedGroupId)}?limit=0`)
    ...
    const accountData = await Promise.all(
      addresses.map(async address => {
        try {
          ...
          return typeof account?.publicKey === 'string' ? account.publicKey : ''
        } catch {
          return ''
        }
      })
  ...
  async function publishQDNResource() {
    ...
    try {
      const { publishes: attachmentPublishes, references: attachmentReferences } =
        await buildAttachmentPayloads(senderName)
      const composedMessageBody =
        isReply && quotedReplyHtml
          ? [value, quotedReplyHtml].filter(Boolean).join('<p><br /></p>')
          : value
      ...
      const recipientAddress =
        typeof recipientNameData?.owner === 'string' ? recipientNameData.owner : ''
      if (!recipientAddress) {
      ...
      const recipientPublicKey =
        typeof recipientAccount?.publicKey === 'string' ? recipientAccount.publicKey : ''
              ...
              <Autocomplete
                ...
                onInputChange={(_, newInputValue, reason) => {
                  ...
                  setSelectedTargetOption(prev => {
                    if (!prev) return null
                    if (normalizeValue(prev.label) === normalizeValue(newInputValue)) {
                      return prev
                    }
                    return null
                  })
                ...
                renderOption={(props, option) => {
                  const typeLabel = option.targetType === 'group' ? 'Group' : 'Name'
                  const sourceLabel =
        ...
        >
          {replyTo && (
            ...
            >
              ...
              >
                ...
                >
                  <Button
                    ...
                    size='small'
                    variant={replyPreviewMode === 'preview' ? 'contained' : 'text'}
                    sx={{
                      minWidth: 'unset',
                      textTransform: 'none',
                      color:
              ...
              >
                {formatFullTimestamp(replyTo?.createdAt)} • {replyTo?.subject || '- no subject -'}
              </Typography>
              <Typography
                sx={{
                  fontSize: '0.8rem',
              ...
              >
                Your reply is written above. The original message is appended beneath it when sent.
              </Typography>
          ...
          >
            <TextEditor
              ...
              placeholder={replyTo ? 'Write your reply here' : 'Write your message here'}
        ...
        >
          ...
          >
            ...
            {isGroupTarget && !replyTo ? (
              <CreateThreadIcon color='currentColor' opacity={1} height='25px' width='25px' />
            ) : (
              <SendNewMessage color='currentColor' opacity={1} height='25px' width='25px' />
            )}
          </NewMessageSendButton>
        </Box>
      </InstanceFooter>
        ...
        <MultiplePublish
          ...
          onSubmit={() => {
            const successMessage =
              pendingPublishType === 'thread' ? 'Thread published' : 'Message sent'
            dispatch(
              setNotification({
                msg: successMessage,
                alertType: 'success'
